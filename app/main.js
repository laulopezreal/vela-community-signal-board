...
const DEMO_SIGNALS = [
  {
    title: 'AI startup pilot request from health founder cohort',
    source: 'Founder Slack',
    category: 'Opportunity',
    urgency: 5,
    relevance: 5,
  ...
  {
    title: 'Grant window opens for local digital-skills community',
  ...
  {
    title: 'Maintainer call for speakers at OSS governance meetup',
    source: 'Discord',
    category: 'Event',
    urgency: 3,
    relevance: 4,
...
function demoSignalItem(demo, offset = 0) {
  return {
    id: makeId(),
    title: demo.title,
    source: demo.source,
...
function resetDemoSession() {
  state.items = DEMO_SIGNALS.map((demo, idx) => demoSignalItem(demo, idx));
  state.search = '';
  ...
  persist();
  render();
  showToast('Clean demo loaded (3 signals)');
}

function loadDemoSignals() {
  const existingKeys = new Set(state.items.map((x) => `${x.title.toLowerCase()}|${x.source.toLowerCase()}`));
  let added = 0;

  DEMO_SIGNALS.forEach((demo) => {
    const key = `${demo.title.toLowerCase()}|${demo.source.toLowerCase()}`;
    if (existingKeys.has(key)) return;

    state.items.push(demoSignalItem(demo, added));
    existingKeys.add(key);
    added += 1;
  });

  if (!added) {
    showToast('Demo set already loaded');
    return;
  }

  persist();
  render();
  showToast(`Loaded ${added} demo signal${added === 1 ? '' : 's'}`);
}
...
async function copyTopActions() {
  ...
  const { lines } = buildBriefLines(items);
...
els.loadDemo?.addEventListener('click', loadDemoSignals);
els.demoReset?.addEventListener('click', resetDemoSession);
