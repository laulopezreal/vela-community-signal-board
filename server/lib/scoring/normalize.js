const { OWNER_MAP, clamp, cleanText, fingerprint, hash, scoreRubric, findPrimaryDimension } = require('./rubric');
...
function normalizeMessage(msg, connector) {
  ...
  const content = cleanText(msg.content || msg.body || msg.subject || '');
...
function parseExport(payload, connector) {
  ...
  if (connector === 'discord') {
    if (Array.isArray(payload?.messages)) {
      for (const m of payload.messages) rows.push({ ...m, _sourceChannel: m.channel || 'Discord #unknown' });
      return rows;
    }
...
module.exports = { toIsoSafe, normalizeMessage, parseExport, clusterDedupe };
